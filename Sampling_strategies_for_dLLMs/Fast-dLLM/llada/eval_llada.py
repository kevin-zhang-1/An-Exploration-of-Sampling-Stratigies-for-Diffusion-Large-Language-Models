        ...
        for batch in tqdm(batched_requests, desc="Generating..."):
            ...
            if self.is_instruct and 'task_id' in req.doc and str(req.doc['task_id']).lower().startswith('humaneval'):
                ...
                if self.show_speed:
                    ...
                    num_nfe += nfe
                batched_generated_answer = [self.tokenizer.decode(generated_answer_ids[i], skip_special_tokens=False) for i in range(len(generated_answer_ids))]
            else:
                ...
                for i in range(len(generated_answer)):
                    ...
                    if self.show_speed:
                        num_tokens += (generated_answer_ids != 126081).sum()
                        num_nfe += nfe
                    generated_answer_i = self.tokenizer.decode(generated_answer_ids, skip_special_tokens=False)
